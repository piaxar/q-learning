...
#            self.trades = np.trunc(self.trades*10000)/10000
#            
#            #self.trades=self.trades.round(5)
#        
...
#        #delta=delta.round(4)
#        #delta = delta.apply(lambda x: x-0.0001 if x>0 else x)
#        
...
#        self.data['pnl'] = self.data['cash']+self.data['value']-initialCash
        ...
        shares=[]
        cash=[]
        for i,sig in enumerate(signal.diff().values):
            if sig==100:
                shares.append(100/price.values[i])
                cash.append(-100)
            elif sig==-100:
                total_shares=sum(shares)
                total_value=sum(shares)*price[i]
                ...
                shares.append(0)
        self.data = pd.DataFrame(index=price.index , columns = ['price','shares','value','cash','pnl'])
        self.data['cash']=initialCash+np.cumsum(cash)
        self.data['price']=price
        self.data['signal']=signal.diff()
        self.data['shares']=shares
        self.data['value']=shares*price
        self.data['pnl']=self.data['cash']+self.data['value']-initialCash                
